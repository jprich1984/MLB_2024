...
def main():
    ...
    teams=pd.read_csv("data/teamStatistics1995-2023.csv") #Read in team statistics csv generated by scrape_mlb.py.  File does not contain
    #team wins

    teams=fixColumns(teams) #fix mislabeld columns
    teams['TEAM']=teams['TEAM'].apply(fixPlayerNames) #clean the team name column

    wins=pd.read_csv('data/WinsOnly.csv') #read in csv that contains team name (code) and amount of wins for a given year
    ...
    team_pitching=pd.read_csv("data/teamPitching1995-2023.csv") #csv generated by scrape_mlb.py
    ...
    division = pd.read_csv('data/mlb_teams.csv') #csv contains division and league information
    ...
    division_recent.to_csv('data/LeagueInfo.csv')
    ...
    pitching.to_csv('data/pitchingAllHistory_clean.csv')
    hitting.to_csv('data/hittingAllHistory_clean.csv')
    final_teams.to_csv('data/TeamWins_UpdatedVersion.csv') #includes years 1995 t0 2023 discluding 2020
...
def lookup(row):
    df=pd.read_csv("data/WinsOnly.csv")
