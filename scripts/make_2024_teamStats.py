...
def main():
    ...
    pitching=pd.read_csv('data/pitchingAllHistory_clean.csv') #Data contains career stats of every pitcher in mlb history
    hitting=pd.read_csv('data/hittingAllHistory_clean.csv') #Data contains career stats of every hitter in mlb history.
    rosters=pd.read_csv("data/PlayerTeamsAll.csv") #File contains this years (2024) rosters with career stats for each team.
    ...
    leagues=pd.read_csv('data/leagueInfo.csv') #contains the current teams with their leagues and divisions
    ...
    team_stats_24.to_csv('data/2024teamStatsProjections2.csv')
...
class LinearModel:
    def __init__(self,df,col):
        self.df=df
        self.col=col
        self.data=pd.read_csv("data/TeamWins_UpdatedVersion.csv")
